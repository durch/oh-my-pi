...
### 5. Update release notes metadata

There is no published workspace changelog for the fork packages. The release source of truth is:
- `npm/oh-omp/package.json` version
- `npm/oh-omp/upstream.json` upstream base metadata
- the GitHub release notes generated by CI

If you want curated notes, add them to the git tag annotation or edit the GitHub release after publish.
